...
def buscar_localizacao_cliente():
    try:
        # Aqui você pode implementar requests.get(...) e result = response.json()
        # Mantendo apenas o tratamento e valores padrão
        result = None  # placeholder, mantém sua lógica de fallback
    ...
    if st.button("Entrar no Painel", use_container_width=True):
        ...
        if ok:
            ...
            if dados_geo and isinstance(dados_geo, dict):
                ...
                estado = dados_geo.get(
                    "estado", dados_geo.get("regiao", "Desconhecido")
                )
        ...
        with col_c2:
            ...
            if st.button(
                ...
                if titulo_com and corpo_com:
                    ...
                    save_db(DB_CLIENTS, st.session_state.db_clients)
                    st.success(
                        f"✅ Enviado para {len(destinatarios)} clientes!"
                    )
                    time.sleep(1)
                    st.rerun()
                else:
